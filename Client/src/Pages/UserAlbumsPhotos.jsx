...
import { getAlbumById, getPhotosPaginated, createPhoto, deletePhoto } from "../Services/albumService";
...
function UserAlbumsPhotos() {
  ...
  const loadUserAlbumAndPhotos = async () => {
    ...
    try {
      ...
      const targetAlbum = await getAlbumById(albumid);
  ...
  const fetchPhotos = async (page, album = selectedAlbum) => {
    ...
    try {
      ...
      const photosData = await getPhotosPaginated(album.id, start, PHOTOS_PER_PAGE);
      if (page === 0) setPhotos(photosData);
      else setPhotos(prev => [...prev, ...photosData]);
  ...
  const confirmDelete = async () => {
    try {
      await deletePhoto(deleteConfirm);
