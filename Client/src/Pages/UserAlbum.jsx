...
import { getAlbums, createAlbum, deleteAlbum } from "../Services/albumService";
...
function UserAlbum() {
  ...
  const loadAlbums = async () => {
    ...
    try {
      ...
      const albumsData = await getAlbums(userId);
      setAlbums(albumsData);
    } catch (error) {
      console.error('Error loading albums:', error);
      if (error.message.includes('401') || error.message.includes('Unauthorized')) {
        showToast("Please login to view albums", "error");
        navigate("/login");
      } else {
        showToast("Failed to load albums. Please try again.", "error");
        setAlbums([]);
      }
    } finally {
      setLoading(false);
  ...
  const handleAddAlbum = async (e) => {
    ...
    try {
      const createdAlbum = await createAlbum({ ...newAlbum, userId });
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (selectedAlbum?.id === deleteConfirm) setSelectedAlbum(null);
